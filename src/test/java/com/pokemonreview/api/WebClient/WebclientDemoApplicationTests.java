...
import com.pokemonreview.api.WebClient.GitHub.GithubRepo;
import com.pokemonreview.api.WebClient.GitHub.GithubRepoRequest;
...
public class WebclientDemoApplicationTests {
	...
	public void testCreateGithubRepository() {
		GithubRepoRequest repoRequest = new GithubRepoRequest("test-webclient-repository", "Repository created for testing WebClient");

		webTestClient.post().uri("/api/repos")
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON)
				.body(Mono.just(repoRequest), GithubRepoRequest.class)
				.exchange()
				.expectStatus().isOk()
...
//	@Order(4)
	public void testEditGithubRepository() {
		GithubRepoRequest newRepoDetails = new GithubRepoRequest("updated-webclient-repository", "Updated name and description");
		webTestClient.patch()
				.uri("/api/repos/{repo}", "test-webclient-repository")
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON)
				.body(Mono.just(newRepoDetails), GithubRepoRequest.class)
				.exchange()
				.expectStatus().isOk()
				.expectHeader().contentType(MediaType.APPLICATION_JSON)
				.expectBody()
